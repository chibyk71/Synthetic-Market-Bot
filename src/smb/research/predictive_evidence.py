...
def audit_dataset(rows: Sequence[EvidenceRow]) -> DatasetAudit:
    total = len(rows)
...
def run_chronological_model(
    ...
    *,
    ...
    naive_total_r = sum(oos_r_all) if oos_r_all else None
    naive_avg_r = (sum(oos_r_all) / len(oos_r_all)) if oos_r_all else None
    ...
    pooled = PooledOOSResult(
        ...
        oos_r_all=oos_r_all,
        naive_total_r=naive_total_r,
        naive_average_r=naive_avg_r,
        notes=notes,
...
def format_predictive_evidence_report(report: PredictiveEvidenceReport) -> str:
    ...
    lines: list[str] = [
        ...
        "## Dataset audit",
        "",
        f"- Total rows analyzed: **{a.total_signals}**",
    ...
    if report.pooled_oos is None:
        ...
        lines.append(f"- Naive total R (all OOS): **{_fmt(p.naive_total_r)}**")
        lines.append("")
